    ...
    <div slot="content">
      <div>
        <v-toolbar flat color="white">
          <v-toolbar-title>Categories CRUD</v-toolbar-title>
          <v-spacer></v-spacer>
  ...
  methods: {
    ...
    deleteItem(item) {
      ...
      confirm("Are you sure you want to delete this item?") &&
        axios.delete(`${this.itemUrl}/${item.slug}`).then(res => {
    ...
    store() {
      if (this.editedIndex > -1) {
        this.updateItem(this.items[this.editedIndex].slug);
    ...
    saveItem() {
      axios
        ...
        .catch(err => {
          flash("Item is not stored", "danger");
        });
    },
    updateItem(slug) {
      axios
        .put(`${this.itemUrl}/${slug}`, { name: this.editedItem.name })
